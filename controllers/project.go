...
func (p *Project) Add(c *gin.Context) {
	if core.IsAjax(c) {
		...
		if name == "" || relation == "" {
			p.Fail(c, config.PromptMsg["parameter_error"])
			return
		}
		if relation == "yes" && len(serverIds) == 0 {
			p.Fail(c, config.PromptMsg["project_server_error"])
			return
		}
		if relation == "yes" && file == nil {
			p.Fail(c, config.PromptMsg["file_upload_error"])
			return
		}
		...
		if ok, str, errorServerList := project.InsertOne(relation, core.StringArrayToInt(serverIds), file); ok {
			...
			if len(errorServerList) > 0 {
				promptMsg := config.PromptMsg[str]
				promptMsg["errorServerList"] = strings.Join(errorServerList, ", ")
				p.Fail(c, promptMsg)
			} else {
				p.Fail(c, config.PromptMsg[str])
			}
		}
	} else {
